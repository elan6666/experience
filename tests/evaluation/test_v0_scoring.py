"""End-to-end tests for the V0 diagnostic scoring slice."""
...
def _write_predictions(
    runs_root: Path, model: str, seed: int, scores: list[float], calendar: tuple[date, ...]
) -> None:
    ...
    frame = PredictionFrame(
        run_id=f"v0-a0-csi300-{model}-seed-{seed:08d}",
        records=records,
    )
    cell_root = runs_root / model / "csi300" / "A0" / f"{seed:08d}"
...
def test_parse_run_id_accepts_v0_and_rejects_other_phases() -> None:
    key = parse_run_id("v0-a0-csi300-timepro-seed-20260719")
    assert key is not None
    assert key.universe == "csi300"
    assert key.model == "timepro"
    assert key.seed == 20260719
    assert key.run_id == "v0-a0-csi300-timepro-seed-20260719"
    assert parse_run_id("v1-a1-csi300-ridge-seed-20260719") is None
    assert parse_run_id("not-a-run-id") is None


def test_load_trading_calendar_filters_to_scored_window(tmp_path: Path) -> None:
...
def test_score_v0_universe_produces_scorecards_and_aggregates(tmp_path: Path) -> None:
    canonical, staged, calendar = _build_canonical(tmp_path)
    runs_root = tmp_path / "runs" / "v0" / "v0"
    _write_predictions(
        runs_root, "alpha", 20260719, [0.10, -0.05, 0.20, -0.10, 0.30, -0.15], calendar
    )
    _write_predictions(
        runs_root, "beta", 20260719, [-0.10, 0.05, -0.20, 0.10, -0.30, 0.15], calendar
    )
    ...
    assert ("alpha", 1) in result.seed_counts
    assert ("beta", 1) in result.seed_counts
    ...
    common_relative = [
        ...
        if card.support is SupportMode.COMMON and card.outcome is OutcomeMode.BENCHMARK_RELATIVE
    ]
    ...
    assert len(alpha_aggregate) == 1
    assert alpha_aggregate[0].seed_count == 1
    assert alpha_aggregate[0].rank_ic_mean is not None
    assert alpha_aggregate[0].rank_ic_std is None


def test_score_v0_universe_detects_prediction_hash_tamper(tmp_path: Path) -> None:
    canonical, staged, calendar = _build_canonical(tmp_path)
    runs_root = tmp_path / "runs" / "v0" / "v0"
    _write_predictions(
        runs_root, "alpha", 20260719, [0.10, -0.05, 0.20, -0.10, 0.30, -0.15], calendar
    )
    manifest_path = next(runs_root.rglob("run_manifest.json"))
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    manifest["prediction_hash"] = "0" * 64
